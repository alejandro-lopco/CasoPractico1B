...
def divide(op_1, op_2):
    try:
        num_1, num_2 = util.convert_to_number(op_1), util.convert_to_number(op_2)
        if num_2 != 0:
            return ("{}".format(int(CALCULATOR.divide(num_1, num_2))), http.client.OK, HEADERS)        
        else:
            return ("La division entre cero no esta permitida", http.client.NOT_ACCEPTABLE, HEADERS)
    except TypeError as e:
        return (str(e), http.client.BAD_REQUEST, HEADERS)
